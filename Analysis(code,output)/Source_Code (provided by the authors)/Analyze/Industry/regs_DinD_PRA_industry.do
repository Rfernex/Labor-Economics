...
args type_data prePRA_variable //default is SSNRA.
if "`type_data'" == ""{
...
label var hours_above_limit "Pre-PRA Workweek"
...
if "`type_data'"~="_NICB" | "`type_data'`lab_results'" ~= "SSNRA_workweek_prePRA"{
	graph export "Figures/regs_DinD_PRA_pretrends`type_data'`lab_results'.pdf", replace
}
...
esttab `indexed_ewemt_models' using "Tables/regs_DinD_PRA_industry`type_data'`lab_results'_earn", `estab_opts' indicate(`r(indicate_fe)') mgroups(`label', pattern(`pattern') prefix(\multicolumn{@span}{c}{) suffix(}) span) keep( PRA_`prePRA_variable' PRA_period `prePRA_variable') order(PRA_period `prePRA_variable' PRA_`prePRA_variable' )
